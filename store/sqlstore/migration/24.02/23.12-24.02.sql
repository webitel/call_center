...
SELECT c.id,
       ...
       c.transfer_from,
       CASE
           WHEN c.parent_id IS NOT NULL AND c.transfer_to IS NULL AND c.id <> call_center.cc_bridged_id(c.parent_id)
               THEN call_center.cc_bridged_id(c.parent_id)
           ELSE c.transfer_to
           END                                                                                                      AS transfer_to,
       ...
       CASE
           ...
           END                                                                                                      AS bill_sec,
       c.sip_code,
       (SELECT json_agg(jsonb_build_object('id', f_1.id, 'name', f_1.name, 'size', f_1.size, 'mime_type', f_1.mime_type,
                                           'start_at', (c.params -> 'record_start'::text)::bigint, 'stop_at',
        ...
        FROM (SELECT f1.id,
              ...
              WHERE f1.domain_id = c.domain_id
                AND NOT f1.removed IS TRUE
                AND f1.uuid::text = c.parent_id::text) f_1)                                                         AS files,
       call_center.cc_get_lookup(cq.id::bigint, cq.name)                                                            AS queue,
       call_center.cc_get_lookup(c.member_id, cm.name)                                                              AS member,
       call_center.cc_get_lookup(ct.id, ct.name)                                                                    AS team,
       ...
       (EXISTS(SELECT 1
               ...
               WHERE c.parent_id IS NULL
                 AND hp.parent_id = c.id
                 AND hp.created_at > c.created_at::date))                                                           AS has_children,
       COALESCE(regexp_replace(cma.description::text, '^[\r\n\t ]*|[\r\n\t ]*$'::text, ''::text, 'g'::text),
       ...
       (SELECT jsonb_agg(x.hi ORDER BY (x.hi -> 'start'::text)) AS res
        FROM (SELECT jsonb_array_elements(chh.hold) AS hi
              ...
              WHERE chh.parent_id = c.id
                AND chh.created_at > c.created_at::date
       ...
       (SELECT jsonb_agg(json_build_object('id', j.id, 'created_at', call_center.cc_view_timestamp(j.created_at),
        ...
        WHERE (j.file_id IN (SELECT f_1.id
                             FROM (SELECT f1.id
                                   ...
                                   WHERE f1.domain_id = c.domain_id
                                     AND NOT f1.removed IS TRUE
                                     AND f1.uuid::text = c.parent_id::text) f_1)))                                  AS files_job,
