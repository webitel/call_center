...
alter table call_center.cc_queue
    add column if not exists form_schema_id integer;
...
CREATE TRIGGER cc_outbound_resource_display_changed_iud
    AFTER INSERT OR DELETE OR UPDATE
    ON call_center.cc_outbound_resource_display
    FOR EACH ROW
EXECUTE FUNCTION call_center.cc_outbound_resource_display_changed();
...
CREATE PROCEDURE call_center.cc_attempt_distribute_cancel(IN attempt_id_ bigint, IN description_ character varying,
                                                          IN next_distribute_sec_ integer, IN stop_ boolean,
                                                          IN vars_ jsonb)
    LANGUAGE plpgsql
AS
$$
...
begin
    ...
    set last_hangup_at = (extract(EPOCH from now()) * 1000)::int8,
        ...
        ready_at       = case
                             when next_distribute_sec_ > 0 then now() + (next_distribute_sec_::text || ' sec')::interval
                             else now() end,
...
CREATE or replace FUNCTION call_center.cc_attempt_end_reporting(attempt_id_ bigint, status_ character varying,
...
CREATE PROCEDURE call_center.cc_call_set_bridged(IN call_id_ character varying, IN state_ character varying,
                                                 IN timestamp_ timestamp with time zone, IN app_id_ character varying,
                                                 IN domain_id_ bigint, IN call_bridged_id_ character varying)
    LANGUAGE plpgsql
AS
$$
declare
...
begin
    ...
    set bridged_id = c.bridged_id,
        ...
        to_number  = case
                         when (cc.direction = 'inbound' and cc.parent_id isnull) or
                              (cc.direction = 'outbound' and cc.gateway_id isnull)
                             then c.number_
                         else to_number end,
        to_name    = case
                         when (cc.direction = 'inbound' and cc.parent_id isnull) or
    ...
    set bridged_id    = c.bridged_id,
        ...
        timestamp     = timestamp_,
        parent_id     = case
                            when c.is_leg_a is true and cc.parent_id notnull and cc.parent_id != c.bridged_id
                                then c.bridged_id
    ...
    from (select b.id,
          ...
          from call_center.cc_calls b
                   ...
                   left join lateral call_center.cc_call_get_owner_leg(b2) b2o on true
          where b.id = call_id_) c
    where c.id = cc.id
    returning cc.transfer_from into transfer_from_;

    update call_center.cc_calls
    set transfer_from = case when id = transfer_from_ then transfer_to_ end,
...
CREATE INDEX cc_agent_state_history_dev_g ON call_center.cc_agent_state_history USING btree (joined_at DESC, agent_id) INCLUDE (state) WHERE (
...
FROM ((SELECT call_center.cc_get_lookup((q_1.id)::bigint, q_1.name) AS queue,
              ...
              CASE
                  WHEN (q_1.type = ANY (ARRAY [1, 6])) THEN (SELECT count(*) AS count
                                                             ...
                                                             WHERE ((a_1_1.queue_id = q_1.id) AND
                                                                    ((a_1_1.state)::text = ANY
                                                                     (ARRAY [('wait_agent'::character varying)::text, ('offering'::character varying)::text])) AND
                                                                    (a_1_1.leaving_at IS NULL)))
       ...
       WHERE (((q_1.team_id IS NULL) OR (a_1.team_id = q_1.team_id)) AND (EXISTS(SELECT qs.queue_id
                                                                                 FROM (call_center.cc_queue_skill qs
                                                                                     JOIN call_center.cc_skill_in_agent csia
                                                                                       ON ((csia.skill_id = qs.skill_id)))
                                                                                 WHERE (qs.enabled AND csia.enabled AND
                                                                                        (csia.agent_id = a_1.id) AND
                                                                                        (qs.queue_id = q_1.id) AND
                                                                                        (csia.capacity >= qs.min_capacity) AND
                                                                                        (csia.capacity <= qs.max_capacity)))))
    ...
    LEFT JOIN LATERAL ( SELECT DISTINCT array_agg(DISTINCT a_1.id)
                                        ...
                                        FILTER (WHERE (((a_1.status)::text = 'online'::text) AND
                                                       (ac.channel IS NULL) AND
                                                       ((ac.state)::text = 'waiting'::text)))                                                                            AS free,
                        ...
                        WHERE ((a_1.domain_id = q.domain_id) AND ((q.team_id IS NULL) OR (a_1.team_id = q.team_id)) AND
                               (qs.skill_id = sia.skill_id) AND (sia.capacity >= qs.min_capacity) AND
                               (sia.capacity <= qs.max_capacity))
...
WHERE ((q.type = ANY (ARRAY [1, 6, 7, 8])) OR ((q.type = 5) AND (NOT q.op)) OR
       (q.op AND (q.type = ANY (ARRAY [2, 3, 4, 5])) AND (r.* IS NOT NULL)));
...
SELECT call_center.cc_get_lookup((q.id)::bigint, q.name)                                                          AS queue,
       ...
       date_part('epoch'::text, sum((t.leaving_at - t.bridged_at))
                                FILTER (WHERE (t.bridged_at IS NOT NULL)))                                        AS bill_sec,
       date_part('epoch'::text, avg((t.leaving_at - t.reporting_at))
                                FILTER (WHERE (t.reporting_at IS NOT NULL)))                                      AS avg_wrap_sec,
       date_part('epoch'::text, avg((t.bridged_at - t.offering_at))
                                FILTER (WHERE (t.bridged_at IS NOT NULL)))                                        AS avg_awt_sec,
       date_part('epoch'::text, max((t.bridged_at - t.offering_at))
                                FILTER (WHERE (t.bridged_at IS NOT NULL)))                                        AS max_awt_sec,
       date_part('epoch'::text, avg((t.bridged_at - t.joined_at))
                                FILTER (WHERE (t.bridged_at IS NOT NULL)))                                        AS avg_asa_sec,
       date_part('epoch'::text, avg((GREATEST(t.leaving_at, t.reporting_at) - t.bridged_at))
                                FILTER (WHERE (t.bridged_at IS NOT NULL)))                                        AS avg_aht_sec,
       q.id                                                                                                       AS queue_id,
       q.team_id
...
alter TABLE call_center.cc_team
    add column if not exists invite_chat_timeout smallint DEFAULT 30 NOT NULL;
...
SELECT c.id,
       ...
       (SELECT jsonb_agg(json_build_object('id', j.id, 'created_at', call_center.cc_view_timestamp(j.created_at),
                                           'action', j.action, 'file_id', j.file_id)) AS jsonb_agg
...
FROM (((((((((((call_center.cc_calls_history c
    LEFT JOIN LATERAL ( SELECT array_agg(f_1.id)                              AS file_ids,
                               json_agg(jsonb_build_object('id', f_1.id, 'name', f_1.name, 'size', f_1.size,
                                                           ...
                                                           ((c.params -> 'record_stop'::text))::bigint, 'transcripts',
                                                           transcripts.data)) AS files
                        FROM ((SELECT f1.id,
                                      f1.size,
                                      f1.mime_type,
                                      f1.name
                               ...
                               FROM storage.files f1
                               WHERE ((f1.domain_id = c.domain_id) AND (NOT (f1.removed IS TRUE)) AND
                                      ((f1.uuid)::text = (c.parent_id)::text))) f_1
                            LEFT JOIN LATERAL ( SELECT json_agg(json_build_object('id', tr.id, 'locale', tr.locale)) AS data
                                                ...
                                                GROUP BY tr.file_id) transcripts ON (true))) f
...
WITH agents AS MATERIALIZED (SELECT a_1.id,
                             ...
                             FROM (((((call_center.cc_agent a_1
                                 ...
                                 LEFT JOIN LATERAL ( SELECT aa.state,
                                                     ...
                                                     WHERE ((aa.agent_id = a_1.id) AND (aa.channel IS NULL) AND
                                                            ((aa.state)::text = ANY
                                                             (ARRAY [('pause'::character varying)::text, ('online'::character varying)::text, ('offline'::character varying)::text])) AND
                                                            (aa.joined_at < (d."from")::timestamp with time zone))
                                 ...
                                 LEFT JOIN LATERAL ( SELECT a2.state,
                                                     ...
                                                     WHERE ((a2.agent_id = a_1.id) AND (a2.channel IS NULL) AND
                                                            ((a2.state)::text = ANY
                                                             (ARRAY [('pause'::character varying)::text, ('online'::character varying)::text, ('offline'::character varying)::text])) AND
                                                            (a2.joined_at > d."to"))
     ...
     d AS MATERIALIZED (SELECT x.agent_id,
                        ...
                        FROM (SELECT a_1.agent_id,
                              ...
                              FROM call_center.cc_agent_state_history a_1,
                                   agents
                              WHERE ((a_1.agent_id = agents.id) AND (a_1.joined_at >= agents."from") AND
                                     (a_1.joined_at <= agents."to") AND (a_1.channel IS NULL) AND
                                     ((a_1.state)::text = ANY
                                      (ARRAY [('pause'::character varying)::text, ('online'::character varying)::text, ('offline'::character varying)::text])))
                              UNION
                              SELECT agents.id,
     ...
     eff AS (SELECT h.agent_id,
             ...
             WHERE ((h.domain_id = agents.domain_id) AND (h.joined_at >= (agents."from")::timestamp with time zone) AND
                    (h.joined_at <= agents."to") AND ((h.channel)::text = 'call'::text))
             GROUP BY h.agent_id),
     chats AS (SELECT cma.agent_id,
               ...
               WHERE ((cma.joined_at >= (agents."from")::timestamp with time zone) AND
                      (cma.joined_at <= agents."to") AND (cma.domain_id = agents.domain_id) AND
                      (cma.bridged_at IS NOT NULL) AND ((cma.channel)::text = 'chat'::text))
     ...
     calls AS (SELECT h.user_id,
                      ...
                      count(*) FILTER (WHERE (((h.direction)::text = 'inbound'::text) AND
                                              (h.bridged_at IS NOT NULL)))                                                                                              AS inbound_bridged,
                      count(*) FILTER (WHERE ((cq.type = 1) AND (h.bridged_at IS NOT NULL) AND
                                              (h.parent_id IS NOT NULL)))                                                                                               AS "inbound queue",
                      count(*)
                      FILTER (WHERE (((h.direction)::text = 'inbound'::text) AND (h.queue_id IS NULL)))                                                                 AS "direct inbound",
                      count(*) FILTER (WHERE ((h.parent_id IS NOT NULL) AND (h.bridged_at IS NOT NULL) AND
                                              (h.queue_id IS NULL) AND
                      ...
                      count(*) FILTER (WHERE (
                              (cq.type = ANY (ARRAY [(0)::smallint, (3)::smallint, (4)::smallint, (5)::smallint])) AND
                      ...
                      avg((h.hangup_at - h.bridged_at)) FILTER (WHERE ((h.bridged_at IS NOT NULL) AND
                                                                       ((h.direction)::text = 'outbound'::text)))                                                       AS "avg bill outbound",
               ...
               WHERE ((h.domain_id = agents.domain_id) AND
                      (h.created_at >= (agents."from")::timestamp with time zone) AND (h.created_at <= agents."to"))
               GROUP BY h.user_id),
     stats AS MATERIALIZED (SELECT s.agent_id,
                                   ...
                                   max(s.joined_at)
                                   FILTER (WHERE ((s.state)::text = 'offline'::text))                                                                  AS logout,
                                   ...
                                   sum(s.dur) FILTER (WHERE (((s.state)::text = 'pause'::text) AND
                                                             ((s.payload)::text = 'Навчання'::text)))                                                  AS study,
                                   sum(s.dur) FILTER (WHERE (((s.state)::text = 'pause'::text) AND
                                                             ((s.payload)::text = 'Нарада'::text)))                                                    AS conference,
                                   sum(s.dur) FILTER (WHERE (((s.state)::text = 'pause'::text) AND
                                                             ((s.payload)::text = 'Обід'::text)))                                                      AS lunch,
                                   sum(s.dur) FILTER (WHERE (((s.state)::text = 'pause'::text) AND
                                                             ((s.payload)::text = 'Технічна перерва'::text)))                                          AS tech
                            FROM (((s
                                     ...
                                     (EXTRACT(epoch FROM (stats.online - COALESCE(stats.lunch, '00:00:00'::interval))) >
                                      (0)::numeric)) THEN (
                                       ((((((COALESCE(EXTRACT(epoch FROM c."Call initiation"), (0)::numeric) +
                                        ...
                                        EXTRACT(epoch FROM
                                                (stats.online - COALESCE(stats.lunch, '00:00:00'::interval)))) *
                                       (100)::numeric)
                               ELSE (0)::numeric
...
begin
    if new.default is distinct from old."default" or new.service is distinct from old."service" then
        ...
        set "default" = false
        where c.domain_id = new.domain_id
          and c.id != new.id
...
alter table call_center.cc_outbound_resource_acl
    alter column grantor drop not null;
alter table call_center.cc_outbound_resource_group_acl
    alter column grantor drop not null;
alter table call_center.cc_queue_acl
    alter column grantor drop not null;
alter table call_center.cc_team_acl
    alter column grantor drop not null;
