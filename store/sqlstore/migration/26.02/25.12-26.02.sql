...
drop function call_center.cc_confirm_agent_attempt;
--
...
create view call_center.cc_agent_in_queue_view
    (queue, priority, type, strategy, enabled, count_members, waiting_members, active_members, queue_id,
     queue_name, team_id, domain_id, agent_id, agents, max_member_limit)
as
SELECT q.queue,
       q.priority,
       q.type,
       ...
       jsonb_build_object('online', COALESCE(array_length(a.agent_on_ids, 1), 0), 'pause',
                          ...
                          'total', COALESCE(array_length(a.total, 1), 0), 'allow_pause',
                          CASE
                            WHEN q.min_online_agents > 0 THEN GREATEST(COALESCE(array_length(a.agent_p_ids, 1), 0) +
                                                                       COALESCE(array_length(a.agent_on_ids, 1), 0) -
                                                                       q.min_online_agents, 0)
                            ELSE NULL::integer
...
FROM (SELECT call_center.cc_get_lookup(q_1.id::bigint, q_1.name)              AS queue,
      ...
      GROUP BY a_1.id, q_1.id, q_1.priority) q
       LEFT JOIN LATERAL ( SELECT DISTINCT array_agg(DISTINCT a_1.id)
                                           ...
                                           array_agg(DISTINCT a_1.id)
                                           FILTER (WHERE a_1.status::text = 'online'::text AND ac.state::text =
                                                                                               'waiting'::text)                                                       AS free,
                                           array_agg(DISTINCT a_1.id)
                                           FILTER (WHERE a_1.status::text = 'online'::text AND ac_call.is_busy)                                                       AS agent_b_ids,
                                           array_agg(DISTINCT a_1.id)                                                                                                 AS total
                           FROM call_center.cc_agent a_1
                                  JOIN call_center.cc_agent_channel ac
                                       ON ac.agent_id = a_1.id AND ac.channel::text = q.chan_name
                                  LEFT JOIN LATERAL ( SELECT bool_or(ac_call_1.state::text <> 'waiting'::text) AS is_busy
                                                      FROM call_center.cc_agent_channel ac_call_1
                                                      WHERE ac_call_1.agent_id = a_1.id
                                                        AND (ac_call_1.channel::text = ANY
                                                             (ARRAY ['call'::text, 'out_call'::text]))) ac_call
                                            ON true
...
WITH agents AS MATERIALIZED (SELECT a_1.id,
     ...
     calls AS (SELECT h.user_id,
                      ...
                      count(*) FILTER (WHERE h.direction::text = 'inbound'::text AND h.bridged_at IS NULL AND
                                             NOT h.hide_missed IS TRUE AND pc.bridged_at IS NULL AND
                                             (cq.type <> ALL (ARRAY [4, 5])))                                                                                   AS missed,
