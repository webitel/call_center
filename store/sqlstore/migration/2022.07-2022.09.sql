...
where m.queue_id = _queue_id
          ...
          and case
                  when _bucket_id isnull then m.bucket_id isnull
                  else m.bucket_id = _bucket_id
end
and (m.expire_at isnull or m.expire_at > now())
        ...
        order by m.bucket_id nulls last,
                 ...
                 m.priority desc,
                 case
                     when coalesce(wait_between_retries_desc, false) then m.ready_at
end
desc nulls last,
                 case
                     when not coalesce(wait_between_retries_desc, false) then m.ready_at
end
asc nulls last,

                 case
                     when coalesce(strategy, 0) = 1 then m.id
end
desc,
                 case
                     when coalesce(strategy, 0) != 1 then m.id
...
CREATE FUNCTION call_center.cc_distribute_inbound_call_to_queue(_node_name character varying, _queue_id bigint,
                                                                _call_id character varying, variables_ jsonb,
                                                                bucket_id_ integer, _priority integer DEFAULT 0,
                                                                _sticky_agent_id integer DEFAULT NULL :: integer) RETURNS record
    LANGUAGE plpgsql
AS
$$
declare
...
WITH queues AS MATERIALIZED (SELECT q_1.domain_id,
                             ...
                             FROM (((WITH mem AS MATERIALIZED (SELECT a.queue_id,
                                                               ...
                                                               WHERE ((a.bridged_at IS NULL) AND
                                                                      (a.leaving_at IS NULL) AND
                             ...
                             WHERE ((m.member_waiting > 0) AND q_1.enabled AND (q_1.type > 0) AND (m.pos = 1) AND
...
WHERE ((q.type = ANY (ARRAY [1, 6, 7])) OR
       ((q.type = 8) AND (GREATEST(((q.lim - COALESCE(l.usage, (0)::bigint)))::integer, 0) > 0)) OR
       ((q.type = 5) AND (NOT q.op)) OR (q.op AND (q.type = ANY (ARRAY [2, 3, 4, 5])) AND (r.* IS NOT NULL)));
...
CREATE FUNCTION call_center.cc_distribute_inbound_chat_to_queue(_node_name character varying, _queue_id bigint,
                                                                _conversation_id character varying, variables_ jsonb,
                                                                bucket_id_ integer, _priority integer DEFAULT 0,
                                                                _sticky_agent_id integer DEFAULT NULL :: integer) RETURNS record
    LANGUAGE plpgsql
AS
...
insert into call_center.cc_member_attempt (domain_id, channel, state, queue_id, member_id, bucket_id, weight,
                                           member_call_id, destination, node_id, sticky_agent_id,
                                           list_communication_id)
values (_domain_id, 'chat', 'waiting', _queue_id, null, bucket_id_, coalesce(_weight, _priority),
        _conversation_id,
        jsonb_build_object('destination', _con_name),
...
return row (
        ...
        jsonb_build_object('inviter_channel_id', _inviter_channel_id) ||
        jsonb_build_object('inviter_user_id', _inviter_user_id),
...
begin
    if
NOT pg_try_advisory_xact_lock(132132118) then
        raise exception 'LOCKED cc_scheduler_jobs';
end if;

with del as (
delete
from call_center.cc_trigger_job
...
insert
into call_center.cc_trigger_job_log (id, trigger_id, state, created_at, started_at, stopped_at, parameters, error,
                                     result, node_id, domain_id)
select id,
       trigger_id,
       state,
       created_at,
...
set schedule_at = t.schedule_at from (select t.id,
                                                  ...
                                                  where t.enabled
                                                    and now() > call_center.cc_cron_next(t.expression,
                                                                                         t.schedule_at + tz.utc_offset)
                                                    and not exists(select 1
                                                                   from call_center.cc_trigger_job tj
                                                                   where tj.trigger_id = t.id and tj.state = 0)
...
CREATE
UNLOGGED TABLE call_center.cc_trigger_job
(
    id         bigint                                 NOT NULL,
...
ALTER COLUMN id SET DEFAULT nextval('call_center.cc_trigger_id_seq'::regclass);
...
ALTER COLUMN id SET DEFAULT nextval('call_center.cc_trigger_acl_id_seq'::regclass);
...
ALTER COLUMN id SET DEFAULT nextval('call_center.cc_trigger_job_id_seq'::regclass);
...
CREATE TRIGGER cc_trigger_set_rbac_acl
    AFTER INSERT
    ON call_center.cc_trigger
    FOR EACH ROW
    EXECUTE FUNCTION call_center.tg_obj_default_rbac('cc_trigger');
...
ALTER TABLE ONLY call_center.cc_trigger_acl
    ADD CONSTRAINT cc_trigger_acl_domain_fk FOREIGN KEY (dc) REFERENCES directory.wbt_domain (dc) ON
DELETE
CASCADE;
...
ALTER TABLE ONLY call_center.cc_trigger_acl
    ADD CONSTRAINT cc_trigger_acl_grantor_id_fk FOREIGN KEY (grantor) REFERENCES directory.wbt_auth (id) ON
DELETE
SET NULL;
...
ALTER TABLE ONLY call_center.cc_trigger_acl
    ADD CONSTRAINT cc_trigger_acl_object_fk FOREIGN KEY (object, dc) REFERENCES call_center.cc_trigger (id, domain_id) ON
DELETE
...
ALTER TABLE ONLY call_center.cc_trigger_acl
    ADD CONSTRAINT cc_trigger_acl_subject_fk FOREIGN KEY (subject, dc) REFERENCES directory.wbt_auth (id, dc) ON
DELETE
CASCADE;
...
CREATE FUNCTION call_center.cc_cron_next_after_now(expression character varying, shed timestamp without time zone,
                                                   nowtz timestamp without time zone) RETURNS timestamp without time zone
    LANGUAGE plpgsql
AS
$$
declare
n timestamp = (call_center.cc_cron_next(expression, shed))::timestamp;
    declare
i int       = 0;
begin
    if
...
from (select t.id,
    ...
    call_center.cc_cron_next_after_now(t.expression, (t.schedule_at):: timestamp,
    (now() at time zone tz.sys_name):: timestamp) new_schedule_at,
    t.domain_id,
...
BEGIN
    if
call_center.cc_cron_valid(NEW.expression) is not true then
        raise exception 'invalid expression %', NEW.expression using errcode = '20808';
end if;

    if
old.enabled != new.enabled or old.expression != new.expression or
                old.timezone_id != new.timezone_id then
select call_center.cc_cron_next_after_now(new.expression, (now() at time zone tz.sys_name):: timestamp,
                                          (now() at time zone tz.sys_name):: timestamp)
...
alter table call_center.cc_trigger
...
WHERE (a.queue_id = q.id))                                                           AS processed
    , count(*)                                                                              AS cnt
    , count(*) FILTER (
WHERE (t.offering_at IS NOT NULL))                                                       AS calls
    , count(*) FILTER (
WHERE ((t.result)::text = 'abandoned'::text))                                            AS abandoned
    , date_part('epoch'::text
    , sum((t.leaving_at - t.bridged_at)) FILTER (WHERE (t.bridged_at IS NOT NULL)))          AS bill_sec
    , date_part('epoch'::text
    , avg((t.leaving_at - t.reporting_at)) FILTER (WHERE (t.reporting_at IS NOT NULL)))      AS avg_wrap_sec
    , date_part('epoch'::text
    , avg((t.bridged_at - t.offering_at)) FILTER (WHERE (t.bridged_at IS NOT NULL)))         AS avg_awt_sec
    , date_part('epoch'::text
    , max((t.bridged_at - t.offering_at)) FILTER (WHERE (t.bridged_at IS NOT NULL)))         AS max_awt_sec
    , date_part('epoch'::text
    , avg((t.bridged_at - t.joined_at)) FILTER (WHERE (t.bridged_at IS NOT NULL)))           AS avg_asa_sec
    , date_part('epoch'::text
    , avg((GREATEST(t.leaving_at
    , t.reporting_at) - t.bridged_at)) FILTER (WHERE (t.bridged_at IS NOT NULL))) AS avg_aht_sec
    , q.id                                                                                  AS queue_id
    , q.team_id
...
WITH res AS (SELECT cqr.queue_id,
                    corg.communication_id,
                    cor.id,
                    cor."limit",
...
SELECT res.queue_id,
       array_agg(DISTINCT
                 ROW (res.communication_id, (res.id)::bigint, res.t, 0)::call_center.cc_sys_distribute_type)                        AS types,
       array_agg(DISTINCT
                 ROW ((res.id)::bigint, ((res."limit" - ac.count))::integer, res.patterns)::call_center.cc_sys_distribute_resource) AS resources,
...
CREATE INDEX cc_agent_state_history_dev_g ON call_center.cc_agent_state_history USING btree (agent_id, joined_at DESC) INCLUDE (state, payload) WHERE (
    (channel IS NULL) AND ((state)::text = ANY
...
CREATE FUNCTION call_center.cc_distribute_direct_member_to_queue(_node_name character varying, _member_id bigint,
                                                                 _communication_id integer, _agent_id bigint)
    RETURNS TABLE
        (
        id                  bigint,
        member_id           bigint,
...
return query with attempts as (
        insert into call_center.cc_member_attempt (state, queue_id, member_id, destination, node_id, agent_id,
                                                   resource_id,
...
alter table call_center.cc_email_profile
    add column imap_host character varying;
alter table call_center.cc_email_profile
    add column smtp_host character varying;
...
while
r notnull and i < days and y < 1000
        loop
...
= y + 1;
            ...
            if
exists(select 1
                           from unnest(r.accepts) x
                           where not x.disabled
                             and x.day = (extract(isodow from d)) - 1)
                    and not exists(
                        select 1
...
alter table flow.acr_routing_scheme
    alter column type SET default 'voice'::character;
alter table flow.acr_routing_scheme
...
FROM (((((((((((((call_center.cc_calls_history c
    ...
    LEFT JOIN LATERAL ( SELECT json_agg(json_build_object('id', tr.id, 'locale', tr.locale, 'file_id', tr.file_id,
                                                          'file', call_center.cc_get_lookup(ff.id, ff.name))) AS data
...
drop
MATERIALIZED VIEW if exists call_center.cc_distribute_stats;
...
AS arr) amd ON (true))
    JOIN LATERAL ( SELECT att.queue_id,
                          ...
                          COALESCE(avg(DISTINCT (round(date_part('epoch'::text,
                                                                 (COALESCE(att.reporting_at, att.leaving_at) - att.offering_at))))::real)
                                   FILTER (WHERE (att.bridged_at IS NOT NULL)),
                          ...
                          count(*) FILTER (WHERE ((ch.answered_at IS NOT NULL) AND
                                                  (((ch.amd_result)::text = ANY (amd.arr)) OR (ch.amd_result IS NULL)))) AS connected_calls,
                          ...
                          count(*) FILTER (WHERE ((ch.answered_at IS NOT NULL) AND (att.bridged_at IS NULL) AND
                                                  (((ch.amd_result)::text = ANY (amd.arr)) OR (ch.amd_result IS NULL)))) AS abandoned_calls,
                          ((count(*) FILTER (WHERE ((ch.answered_at IS NOT NULL) AND
                                                    (((ch.amd_result)::text = ANY (amd.arr)) OR (ch.amd_result IS NULL)))))::double precision /
                           (count(*))::double precision)                                                                 AS connection_rate,
                          CASE
                              WHEN (((count(*) FILTER (WHERE ((ch.answered_at IS NOT NULL) AND
                                                              (((ch.amd_result)::text = ANY (amd.arr)) OR (ch.amd_result IS NULL)))))::double precision /
                                     (count(*))::double precision) > (0)::double precision) THEN (
                                      ((1)::double precision / ((count(*)
                                                                 FILTER (WHERE ((ch.answered_at IS NOT NULL) AND
                                                                                (((ch.amd_result)::text = ANY (amd.arr)) OR (ch.amd_result IS NULL)))))::double precision /
                                                                (count(*))::double precision)) - (1)::double precision)
                              ELSE (((count(*) / GREATEST(count(DISTINCT att.agent_id), (1)::bigint)) -
                                     1))::double precision
                              END                                                                                        AS over_dial,
                          COALESCE(((((count(*)
                                  ...
                                  (((ch.amd_result)::text = ANY (amd.arr)) OR (ch.amd_result IS NULL)))),
                                                                                 0))::double precision) *
                   ...
                   WHERE (((att.channel)::text = 'call'::text) AND (att.joined_at > (now() -
                                                                                     ((COALESCE(((q.payload -> 'statistic_time'::text))::integer, 60) ||
                                                                                       ' min'::text))::interval)) AND
                          (att.queue_id = q.id) AND (att.domain_id = q.domain_id))
                   GROUP BY att.queue_id, att.bucket_id) s ON ((s.queue_id IS NOT NULL)))
WHERE ((q.type = 5) AND q.enabled)
WITH NO DATA;
refresh
materialized view call_center.cc_distribute_stats;

alter table call_center.cc_email
    add column attachment_ids bigint[];
...
FROM (((((((((((((call_center.cc_queue q
    ...
    LEFT JOIN LATERAL ( SELECT count(*) AS cnt
                        FROM call_center.cc_member_attempt a
                        WHERE ((a.queue_id = q.id) AND (a.leaving_at IS NULL) AND
                               ((a.state)::text <> 'leaving'::text))) act ON (true));
...
CREATE FUNCTION call_center.cc_distribute_inbound_call_to_agent(_node_name character varying,
                                                                _call_id character varying, variables_ jsonb,
                                                                _agent_id integer DEFAULT NULL :: integer) RETURNS record
    LANGUAGE plpgsql
AS
$$
declare
...
_call.domain_id != _domain_id then
    ...
    if
_team_id_ isnull then
        raise exception 'not found agent';
end if;

    if
not _a_status = 'online' then
        raise exception 'agent not in online';
end if;

    if
not _a_channel isnull then
...
insert into call_center.cc_member_attempt (domain_id, state, team_id, member_call_id, destination, node_id,
                                           agent_id,
                                           parent_id)
...
CREATE FUNCTION call_center.cc_set_active_members(node character varying)
    RETURNS TABLE
        (
        id                    bigint,
        member_id             bigint,
...
return query update call_center.cc_member_attempt a
        set state = case
                        when c.queue_type = 4 then 'offering'
                        else 'waiting'
end
,node_id = node
        ...
        from (select c.id,
              ...
              order by cq.priority desc, c.weight desc
                  for update of c, cm, cq skip locked) c
...
alter table storage.import_template
...
ALTER COLUMN id SET DEFAULT nextval('storage.import_template_acl_id_seq'::regclass);
...
CREATE TRIGGER import_template_set_rbac_acl
    AFTER INSERT
    ON storage.import_template
    FOR EACH ROW
    EXECUTE FUNCTION storage.tg_obj_default_rbac('import_template');
...
ALTER TABLE ONLY storage.import_template_acl
    ADD CONSTRAINT import_template_acl_domain_fk FOREIGN KEY (dc) REFERENCES directory.wbt_domain (dc) ON
DELETE
CASCADE;
...
ALTER TABLE ONLY storage.import_template_acl
    ADD CONSTRAINT import_template_acl_grantor_id_fk FOREIGN KEY (grantor) REFERENCES directory.wbt_auth (id) ON
DELETE
SET NULL;
...
ALTER TABLE ONLY storage.import_template_acl
    ADD CONSTRAINT import_template_acl_object_fk FOREIGN KEY (object, dc) REFERENCES storage.import_template (id, domain_id) ON
DELETE
CASCADE DEFERRABLE INITIALLY DEFERRED;
...
ALTER TABLE ONLY storage.import_template_acl
    ADD CONSTRAINT import_template_acl_subject_fk FOREIGN KEY (subject, dc) REFERENCES directory.wbt_auth (id, dc) ON
DELETE
CASCADE;
...
CREATE FUNCTION call_center.appointment_widget(_uri character varying)
    RETURNS TABLE
        (
        profile jsonb,
        list    jsonb
...
AS
$$
with profile as (select (config['queue'] ->> 'id')::int                       as queue_id,
                 ...
                 where b.uri = _uri
                   and b.enabled
                 limit 1)
   , d as materialized (select q.queue_id,
                               q.duration,
                               q.available_agents,
                               x,
                        ...
                        from profile q,
                             flow.calendar_day_range(q.calendar_id, least(q.days, 7)) x
                                 ...
                                 select (x + (y.start_time_of_day || 'm')::interval)::timestamp as ss,
                                        case
                                            when date_bin(q.duration,
                                                          (x + (y.end_time_of_day || 'm')::interval)::timestamp,
                                                          x::timestamp) < (x + (y.end_time_of_day || 'm')::interval)::timestamp
                                                then date_bin(q.duration,
                                                              (x + (y.end_time_of_day || 'm')::interval)::timestamp,
                                                              x::timestamp) - q.duration
                                            else date_bin(q.duration,
                                                          (x + (y.end_time_of_day || 'm')::interval)::timestamp,
                                                          x::timestamp) - q.duration end        as se
                                 ) dy on true)
   , min_max as materialized (select queue_id,
                                     x,
                                     duration,
                                     min(ss) min_ss,
                                     max(se) max_se
                              from d
                              group by 1, 2, 3)
   , res as materialized (select mem.*
                          from min_max
                                   left join lateral (
                              select date_bin(min_max.duration, coalesce(ready_at, created_at),
                                              coalesce(ready_at, created_at)::date)::timestamp d,
                                     count(*)                                                  cnt
                              from call_center.cc_member m
   ...
   , list as (select d.*,
                     ...
                     xx,
                     case
                         when xx < now() or coalesce(res.cnt, 0) >= d.available_agents then true
                         else false end as reserved
              from d
                       left join generate_series(d.ss, d.se, d.duration) xx on true
                       left join res on res.d = xx
              limit 10080)
   , ranges AS (select to_char(list.x::date, 'YYYY-MM-DD')::text as date,
                       jsonb_agg(
                               jsonb_build_object('time', to_char(list.xx::time, 'HH24:MI'), 'reserved', list.reserved)
                               order by list.x, list.xx)         as times
                from list
                group by 1)
...
create
or replace function call_center.cc_distribute_inbound_call_to_queue(_node_name character varying, _queue_id bigint,
                                                                        _call_id character varying, variables_ jsonb,
                                                                        bucket_id_ integer, _priority integer DEFAULT 0,
                                                                        _sticky_agent_id integer DEFAULT NULL::integer) returns record
    language plpgsql
...
set type = u.s
    from (
         --chat
         select *, 'chat' as s
         from (select distinct flow_id as schema_id
               from chat.bot
               union
               distinct
               select schema_id
               from flow.acr_chat_plan) t
         where schema_id notnull

         union all
         --service
         select *, 'service'
         from (select distinct after_schema_id as schema_id
         ...
         --form
         select *, 'processing'
         from (select distinct form_schema_id as schema_id
               from call_center.cc_queue
               where form_schema_id notnull) t

         union all

         --call
         select *, 'call'
         from (select distinct scheme_id
...
where s.id = u.schema_id
  and (s.type isnull or not s.type in ('default', 'voice', 'service', 'processing', 'chat'));
...
CREATE
or replace PROCEDURE call_center.cc_scheduler_jobs()
    LANGUAGE plpgsql
AS
$$
begin
...
from (select t.id,
    ...
    t.domain_id,
    (t.schedule_at):: timestamp as                                                   old_schedule_at,
    (now() at time zone tz.sys_name):: timestamp - (t.schedule_at):: timestamp <
    interval '5m'                                                                    valid
    from call_center.cc_trigger t
    inner join flow.calendar_timezones tz on tz.id = t.timezone_id
    where t.enabled
...
set queue_id   = _attempt.queue_id,
    team_id    = _team_id_,
    attempt_id = _attempt.id,
    payload    = coalesce(variables_, '{}') || cc_calls.payload,
    grantee_id = _grantee_id
where id = _call_id
    returning * into _call;
