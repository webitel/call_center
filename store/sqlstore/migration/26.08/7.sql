...
create or replace view call_center.cc_calls_history_list as
  SELECT c.id,
    ...
    c.direction,
    c.destination::text AS destination,
    ...
    ( SELECT json_agg(jsonb_build_object('id', f_1.id, 'name', f_1.name, 'size', f_1.size, 'mime_type', f_1.mime_type, 'start_at',
                CASE
                    WHEN f_1.channel::text = 'call'::text AND NOT f_1.mime_type::text ~~ 'image%'::text THEN COALESCE(NULLIF((f_1.custom_properties ->> 'start_time'::text)::bigint, 0), NULLIF((c.params -> 'record_start'::text)::bigint, 0), NULLIF(call_center.cc_view_timestamp(c.bridged_at), 0), call_center.cc_view_timestamp(c.answered_at))
                    ELSE COALESCE(NULLIF((f_1.custom_properties ->> 'start_time'::text)::bigint, 0), f_1.created_at)
                END, 'stop_at',
                CASE
                    WHEN f_1.channel::text = 'call'::text AND NOT f_1.mime_type::text ~~ 'image%'::text THEN COALESCE(NULLIF((f_1.custom_properties ->> 'end_time'::text)::bigint, 0), NULLIF((c.params -> 'record_stop'::text)::bigint, 0), call_center.cc_view_timestamp(c.hangup_at))
    ...
    COALESCE(c.params ->> 'ua'::text, ''::text) AS user_agent
