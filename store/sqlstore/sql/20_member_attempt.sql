create table IF NOT EXISTS cc_member_attempt
(
...
create index IF NOT EXISTS cc_member_attempt_communication_id_hangup_time_index
	on cc_member_attempt (communication_id, hangup_time);

create index IF NOT EXISTS cc_member_attempt_hangup_at_queue_id_index
	on cc_member_attempt (hangup_at, queue_id);

create index IF NOT EXISTS cc_member_attempt_state_index_test
	on cc_member_attempt (state)
	where (hangup_at = 0);
