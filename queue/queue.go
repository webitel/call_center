...
func (queue *BaseQueue) StartProcessingForm(attempt *Attempt) {
	if queue.formSchemaId != nil && queue.Processing() {
...
func (tm *agentTeam) Cancel(attempt *Attempt, agent agent_manager.AgentObject) {
	...
	res, err := tm.teamManager.store.Member().SetAttemptAbandonedWithParams(attempt.Id(), attempt.maxAttempts, attempt.waitBetween, nil, attempt.perNumbers)
	if err != nil {
		wlog.Error(err.Error())
